...
__global__ void gpu_matrix_mult_shared_conflict(int *d_a, int *d_b, int *d_result, int m, int n, int k) 
{
    __shared__ int tile_a[BLOCK_SIZE][BLOCK_SIZE];
    ...
    for (int sub = 0; sub <= n/BLOCK_SIZE; ++sub) 
    {
        ...
        /* 在此之上，与无conflict一样 */
        // 制造conflict，将share变量以转置的方式存储 多个线程访问同一个bank
        tile_a[threadIdx.x][threadIdx.y] = row<m && (sub * BLOCK_SIZE + threadIdx.x)<n? d_a[idx]:0;
        idx = (sub * BLOCK_SIZE + threadIdx.y) * k + col; // n
        tile_b[threadIdx.x][threadIdx.y] = col<k && (sub * BLOCK_SIZE + threadIdx.y)<n? d_b[idx]:0;
        /*
        一个共享块的例子：
        针对于tile_a
        共享变量的下标                              原数据下标:
        (0,0) (0,1) ... (0,15)                    0 n ... 15n
        (1,0) ...       (1,15)     --------->     1 n+1 ... 15n+1
        ...                                       ...
        (15,0) ...      (15,15)                      15 n+15 .. 15n+15
        */
        
        __syncthreads();
        for (int k = 0; k < BLOCK_SIZE; ++k) 
        {
            tmp += tile_a[k][threadIdx.y] * tile_b[threadIdx.x][k];
        }
        __syncthreads();
    }
    if(row < m && col < k)
...
int main(int argc, char const *argv[])
{
    ...
    int *h_a, *h_b, *h_c, *h_cc, *h_cs, *h_csc;
    ...
    CHECK(cudaMallocHost((void **) &h_csc, sizeof(int)*m*k));  // conflict
    
    cudaEvent_t start, stop,stop_share, stop_share_conflict;
    CHECK(cudaEventCreate(&start));
    CHECK(cudaEventCreate(&stop));
    CHECK(cudaEventCreate(&stop_share));
    CHECK(cudaEventCreate(&stop_share_conflict));
    ...
    int *d_a, *d_b, *d_c, *d_c_share, *d_c_share_conflict;
    ...
    CHECK(cudaMalloc((void **) &d_c_share_conflict, sizeof(int)*m*k));
    ...
    // conflict 
    gpu_matrix_mult_shared_conflict<<<dimGrid, dimBlock>>>(d_a, d_b, d_c_share_conflict, m, n ,k);
    CHECK(cudaMemcpy(h_csc, d_c_share_conflict, (sizeof(int)*m*k), cudaMemcpyDeviceToHost));
    
    CHECK(cudaEventRecord(stop_share_conflict));
    CHECK(cudaEventSynchronize(stop_share_conflict));
    
    
    float elapsed_time, elapsed_time_share, elapsed_time_share_conflict;
    CHECK(cudaEventElapsedTime(&elapsed_time, start, stop));
    CHECK(cudaEventElapsedTime(&elapsed_time_share, stop, stop_share));
    CHECK(cudaEventElapsedTime(&elapsed_time_share_conflict, stop_share, stop_share_conflict));
    printf("Time_global = %g ms.\n", elapsed_time);
    printf("Time_share = %g ms.\n", elapsed_time_share);
    printf("Time_share = %g ms.\n", elapsed_time_share_conflict);

    CHECK(cudaEventDestroy(start));
    CHECK(cudaEventDestroy(stop));    
    CHECK(cudaEventDestroy(stop_share));
    CHECK(cudaEventDestroy(stop_share_conflict));
    ...
    { 
        for (int j = 0; j < k; ++j)
        {
            if(fabs(h_csc[i*k + j] - h_cc[i*k + j])>(1.0e-10))
            {
                printf("hcs: %d hc: %d  ",h_csc[i*k + j], h_c[i*k + j]);
                ok = 0;
            }
        }
    }
